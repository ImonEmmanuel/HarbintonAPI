...
namespace Harbinton.API.Persitence.Contract.Implementation
{
    public class TransactionRepository : ITransactionRepository
    {
        ...
        public async Task<ResponseModel> BillPayment(string accountNumber, BillDto bill)
        {
            ...
            if (accountFrom.Amount < bill.Amount)
            {
                return await TransactionResponse("Failed", bill.BillType,
                        "Insufficient Balance Rety with a less Amount or fund account", transact, HttpStatusCode.NotFound);
            }
